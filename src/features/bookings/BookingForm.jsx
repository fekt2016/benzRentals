...
import { useCreateBooking, useGetCarBookings } from "./useBooking";
import { useNavigate } from "react-router-dom";
import { useAvailableProfessionalDrivers } from "../../features/drivers/useProfessionalDriver";
import { devices } from "../../styles/GlobalStyles";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
import { pulse, slideUp, shake } from '../../styles/animations';
import { getCurrentTimeInStLouis, generateTimeSlots,  USA_CONFIG, DEFAULT_TIME_SLOTS  } from '../../utils/helper'
import {formatTimeForDisplay} from '../../utils/helper'
...
const BookingForm = ({ car, licenses, drivers }) => {
  ...
  // Fetch available professional drivers when carWithDriver is enabled and dates are selected
 



  // Auto-select first available driver when carWithDriver is enabled
  ...
  // Real-time driver request (Phase 2 feature)
  ...
  const validateForm = useCallback(() => {
    ...
    // Driver and license validation (skip if real-time driver request is selected)
    if (!requestDriver && !form.selectedDriver) {
    ...
    // Form validity check
    // Phase 2: Real-time driver request skips driver validation, only needs dates and terms
    ...
    // Phase 2: If requestDriver is true, only validate dates and terms (skip driver validation)
    // Otherwise, require all validations including driver info
    const hasNoErrors = requestDriver ? true : Object.keys(errors).length === 0;
    setIsFormValid(hasValidDates && acceptedTerms && hasNoErrors);
  }, [form, licenseFile, hasBookingConflict, licenses, getConflictingDates, getYesterday, getToday, acceptedTerms, requestDriver]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("acceptedTerm", acceptedTerms)

      // Real-time driver request (Phase 2 feature)
      if (requestDriver) {
        formData.append("requestDriver", "true");
      } else if (form.selectedDriver) {
      ...
      // Backend returns: { status: "success", message: "...", data: { _id: "...", ... } }
      const bookingId = booking?.data?.data?._id || booking?.data?._id || booking?.data?.id || booking?._id;
      const bookingData = booking?.data?.data || booking?.data || booking;
      
      // ============================================
      // 🎯 PHASE 2 FEATURES - COMPREHENSIVE LOGGING
      // ============================================
      console.log("==========================================");
      ...
      console.log("  - Car:", car?.model || car?.name);
      ...
      console.log("  - Driver Service Fee (Est.): $", requestDriver ? driverServiceFee.toFixed(2) + " (will be confirmed when driver accepts)" : driverServiceFee.toFixed(2));
      console.log("  - Grand Total: $", grandTotal.toFixed(2));
      console.log("");
      console.log("🚗 Driver Information:");
      console.log("  - Request Driver (Real-time):", requestDriver);
      if (requestDriver) {
        console.log("  - ⚡ Real-time driver request will be sent to all available drivers");
        console.log("  - First driver to accept will be assigned automatically");
      } else if (form.selectedDriver) {
        console.log("  - Existing Driver Selected:", form.selectedDriver);
      } else if (form.selectedLicense) {
        console.log("  - Existing License Selected:", form.selectedLicense);
      } else if (licenseFile) {
        console.log("  - New License Uploaded:", licenseFile.name);
        console.log("  - Driver Name:", form.driverName || "N/A");
      }
      console.log("");
      console.log("🎯 Phase 2 Features Status:");
      console.log("  - Request Driver (Real-time):", bookingData?.requestDriver || false);
      console.log("  - Driver Request Status:", bookingData?.driverRequestStatus || "N/A");
      console.log("  - Driver Assigned:", bookingData?.driverAssigned || false);
      console.log("  - Accepted Driver:", bookingData?.acceptedDriver || "N/A");
      console.log("  - Booking Status:", bookingData?.status || "N/A");
      console.log("");
      console.log("💳 Payment Information:");
      console.log("  - Payment Status:", bookingData?.paymentStatus || "unpaid");
      console.log("  - Next Step: Proceed to Checkout");
      console.log("");
      console.log("📝 Next Steps:");
      console.log("  1. Navigate to Checkout Page");
      console.log("  2. Apply coupon code (if available)");
      console.log("  3. Complete payment");
      console.log("  4. Receive booking confirmation");
      if (bookingData?.requestDriver) {
        console.log("  5. ⚠️  Waiting for driver assignment via Socket.io");
      }
      console.log("==========================================");
      
      if (!bookingId) {
        console.error('[BookingForm] No booking ID in response. Full response:', JSON.stringify(booking, null, 2));
        throw new Error('Booking created but could not retrieve booking ID. Please check your bookings page.');
      }
      
      // ✅ Phase 2: Real-time Professional Driver Flow
      // Navigate to checkout with requestDriver flag if applicable
      navigate(PATHS.CHECKOUT, { 
        state: { 
          bookingId,
          requestDriver: requestDriver || false // Pass requestDriver flag to checkout
        } 
      });
    } catch (error) {
      console.error("[BookingForm] Booking failed:", {
        error,
      ...
      if (errorMessage?.includes("CORS") || errorMessage?.includes("cors")) {
        errorMessage = "CORS error: Please check your backend server configuration and ensure your frontend origin is allowed.";
      } else if (error?.response?.status === 403) {
        errorMessage = "You don't have permission to create a booking. Please ensure you are logged in.";
      } else if (error?.response?.status === 401) {
        errorMessage = "Your session has expired. Please log in again.";
      } else if (error?.response?.status === 400) {
        errorMessage = errorMessage || "Invalid booking data. Please check your inputs.";
      } else if (error?.code === "ERR_NETWORK" || !error?.response) {
        errorMessage = `Network error: Unable to connect to the server. Please ensure the backend is running on ${import.meta.env.VITE_API_IP || 'localhost'}:${import.meta.env.VITE_API_PORT || '3001'}.`;
      }
      
      if (errorMessage?.includes("already booked") || errorMessage?.includes("conflict") || 
          errorMessage?.includes("available") || errorMessage?.includes("overlap")) {
        setFormErrors(prev => ({ ...prev, submit: errorMessage || "This time slot is no longer available. Please select different dates/times." }));
        await refetchBookings();
      } else {
  ...
  // Calculate estimated driver service fee: $35/hour
  // This is an estimate - actual fee will be calculated when driver accepts
  const calculateDriverServiceFee = useCallback(() => {
    if (!requestDriver || !form.pickupDate || !form.returnDate || !form.pickupTime || !form.returnTime) {
      return 0;
    }
    ...
    try {
      ...
      console.error("[BookingForm] Error calculating driver service fee:", error);
      return 0;
    }
  }, [requestDriver, form.pickupDate, form.returnDate, form.pickupTime, form.returnTime]);

  const driverServiceFee = calculateDriverServiceFee();
  const grandTotal = totalPrice + depositAmount + driverServiceFee;
  ...
  return (
    <FormContainer onSubmit={handleSubmit}>
      ...
      <FormSection>
        ...
        {/* Real-Time Driver Request Option (Phase 2) */}
        {totalDays > 0 && (
          <FormSection>
            <CheckboxGroup>
              <CheckboxInput
                ...
                onChange={(e) => {
                  setRequestDriver(e.target.checked);
                  // Clear driver-related form fields when toggling
                  if (e.target.checked) {
                    setForm(prev => ({
                      ...prev,
                      selectedDriver: null,
                ...
                disabled={isCreatingBooking || isBookingsLoading || !form.pickupDate || !form.returnDate}
              />
              <CheckboxLabel htmlFor="requestDriver">
                <strong>Request Professional Driver (Real-Time)</strong>
            ...
            {requestDriver && (
              <div style={{ padding: "0.5rem 0 0 1.5rem", fontSize: "0.9rem", color: "var(--primary)" }}>
                ⚡ A driver request will be sent to all available drivers. The first driver to accept will be assigned to your booking.
              </div>
            )}
          </FormSection>
        )}
        ...
        {totalDays > 0 && !hasPastDates() && (
          <PriceSummary>
            ...
            {requestDriver && driverServiceFee > 0 && (
            <SummaryItem>
                <span>Professional Driver Service (Est.)</span>
                <span style={{ color: "var(--text-muted)" }}>
                  ${driverServiceFee.toFixed(2)}
                  <span style={{ fontSize: "0.85rem", marginLeft: "4px", fontStyle: "italic" }}>
                    ($35/hour)
                  </span>
                </span>
              </SummaryItem>
            ...
            <SummaryItem>
                ...
                <span style={{ color: "var(--text-muted)", fontStyle: "italic" }}>Select dates to see estimate</span>
              </SummaryItem>
            )}
            <SummaryItem>
      ...
      {/* Driver Selection - Only show if real-time driver request is NOT selected */}
      {!requestDriver && verifiedDrivers.length > 0 && (
        <FormSection>
      ...
      {/* License Selection - Only show if real-time driver request is NOT selected */}
      {!requestDriver && hasExistingLicenses && !form.selectedDriver && (
        <FormSection>
      ...
      {/* File Uploads - Only show if real-time driver request is NOT selected */}
      {!requestDriver && !form.selectedDriver && (
        <FormSection>
          <SectionTitle>Upload Documents</SectionTitle>

          {/* Driver Name - Only show if not using real-time driver request */}
          {!requestDriver && (
            <DriverNameSection>
              <FormGroup>
      ...
      >
        {isCreatingBooking || hasBeenSubmitted ? (
          ...
          <>
            Reserve Now - $
            {grandTotal ? grandTotal.toFixed(2) : car.pricePerDay.toFixed(2)}
          </>
        )}
      </SubmitButton>
    </FormContainer>
