...
const useAuthForm = () => {
  const [formData, setFormData] = React.useState({
    ...
    fullName: "",
    dateOfBirth: "",
    role: "user", // Default to "user", can be "user" or "driver"
  ...
  const handleOtpSuccess = () => {
    ...
    if (token) {
      ...
      // If no token yet, wait a bit for it to be set
      setTimeout(() => {
        const retryToken = Cookies.get("token");
        if (retryToken) {
          try {
            const decoded = jwtDecode(retryToken);
            const userRole = decoded.role;
                ...
                <Form onSubmit={handleSubmit}>
                  {isRegistering && (
                    <>
                      {/* Role Selection */}
                      <FormField>
                        <RoleSelectionLabel>I want to register as:</RoleSelectionLabel>
                        <RoleSelectionContainer>
                          <RoleOption
                            ...
                            disabled={isLoading}
                          >
                            <RoleIcon $selected={formData.role === "user"}>
                              <FiUser />
                            </RoleIcon>
                            <RoleContent>
                              <RoleTitle>Customer</RoleTitle>
                              <RoleDescription>Rent luxury vehicles</RoleDescription>
                            </RoleContent>
                            {formData.role === "user" && <RoleCheckmark>✓</RoleCheckmark>}
                          </RoleOption>
                          ...
                          <RoleOption
                            ...
                            disabled={isLoading}
                          >
                            <RoleIcon $selected={formData.role === "driver"}>
                              <FiShield />
                            </RoleIcon>
                            <RoleContent>
                              <RoleTitle>Professional Driver</RoleTitle>
                              <RoleDescription>Accept ride requests & earn</RoleDescription>
                            </RoleContent>
                            {formData.role === "driver" && <RoleCheckmark>✓</RoleCheckmark>}
                          </RoleOption>
                        </RoleSelectionContainer>
                      </FormField>
...
const RoleSelectionLabel = styled.label`
  display: block;
  font-size: var(--text-sm);
  font-weight: var(--font-semibold);
  color: var(--text-primary);
...
const RoleOption = styled.button`
  ...
  border: 2px solid ${(props) => 
    props.$selected ? "var(--primary)" : "var(--gray-300)"};
  border-radius: var(--radius-lg);
  background: ${(props) => 
...
const RoleIcon = styled.div`
  ...
  border-radius: var(--radius-lg);
  background: ${(props) => 
    props.$selected ? "var(--primary)" : "var(--gray-100)"};
  color: ${(props) => 
    props.$selected ? "var(--white)" : "var(--text-secondary)"};
  display: flex;
  align-items: center;
  justify-content: center;
